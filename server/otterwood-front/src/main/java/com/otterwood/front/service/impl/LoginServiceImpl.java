...
import com.otterwood.common.token.FrontTokenComponent;
...
import com.otterwood.front.service.LoginService;
...
public class LoginServiceImpl implements LoginService {
    ...
    public LoginResponse login(LoginRequest loginRequest) {
        ...
        if (ObjectUtil.isNull(user)) {
            throw new OtterwoodException("此账号未注册");
        }
        if (!user.getStatus()) {
            throw new OtterwoodException("此账号被禁用");
        }

        // 校验密码
        String password = OtterwoodUtil.encryptPassword(loginRequest.getPassword(), loginRequest.getPhone());
        if (!user.getPwd().equals(password)) {
            throw new OtterwoodException("密码错误");
        }
